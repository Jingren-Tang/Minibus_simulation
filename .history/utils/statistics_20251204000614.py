...
class Statistics:
    ...
    def record_passenger(self, passenger: 'Passenger') -> None:
        ...
        try:
            ...
            if passenger.board_time is not None:
                wait_time = passenger.board_time - passenger.appear_time
            
            if passenger.board_time is not None and passenger.arrival_time is not None:
                travel_time = passenger.arrival_time - passenger.board_time
        ...
        try:
            ...
            if vehicle_id not in self.vehicle_records:
                self.vehicle_records[vehicle_id] = {
                    "vehicle_type": "Bus",  # Will be updated when we add minibuses
                    "route": [],
    ...
    def create_dashboard(self, output_file: str = "dashboard.html") -> None:
        ...
        Includes:
            ...
            - Occupancy timeline
            - Service rate chart
            - Interactive filters (optional, if using plotly)
        
        Note: This is a basic implementation using matplotlib plots embedded in HTML.
        For true interactivity, consider using plotly or bokeh.
        """
        logger.info("Creating HTML dashboard...")
        
        try:
            # Generate all plots
            plot_dir = Path(output_file).parent / "plots"
            plot_dir.mkdir(parents=True, exist_ok=True)
            
            wait_plot = plot_dir / "wait_time_dist.png"
            occupancy_plot = plot_dir / "occupancy_timeline.png"
            service_plot = plot_dir / "service_rate_hourly.png"
            
            self.plot_wait_time_distribution(str(wait_plot))
            self.plot_occupancy_over_time(output_file=str(occupancy_plot))
            self.plot_service_rate_by_hour(str(service_plot))
    ...
    <div class="container">
        ...
        <div class="metrics-grid">
            <div class="metric-card">
                <h3>Service Rate</h3>
                <div class="value">{passenger_metrics.get('service_rate', 0):.1f}%</div>
            </div>
            <div class="metric-card">
                <h3>Total Passengers</h3>
                <div class="value">{passenger_metrics.get('total_passengers', 0)}</div>
            </div>
            <div class="metric-card">
                <h3>Avg Wait Time</h3>
                <div class="value">{passenger_metrics.get('avg_wait_time', 0)/60:.1f} min</div>
            </div>
            <div class="metric-card">
                <h3>Total Vehicles</h3>
                <div class="value">{system_metrics.get('total_vehicles', 0)}</div>
            </div>
        </div>
        
        <h2>Wait Time Distribution</h2>
        <div class="plot-container">
            <img src="plots/wait_time_dist.png" alt="Wait Time Distribution">
        </div>
        
        <h2>Vehicle Occupancy Timeline</h2>
        <div class="plot-container">
            <img src="plots/occupancy_timeline.png" alt="Occupancy Timeline">
        </div>
        
        <h2>Hourly Service Rate</h2>
        <div class="plot-container">
            <img src="plots/service_rate_hourly.png" alt="Service Rate by Hour">
        </div>
