...
class Statistics:
    ...
    def calculate_vehicle_metrics(self) -> Dict[str, Dict]:
        ...
        Returns:
            Dictionary with vehicle_id as key:
            {
                "BUS_1": {
                    ...
                    "avg_occupancy": float,
                    "max_occupancy": int,
                    "occupancy_rate": float,  # avg_occupancy / capacity (assuming capacity=80)
                    "total_boardings": int,
        ...
        try:
            ...
            for vehicle_id, record in self.vehicle_records.items():
                ...
                # Assume default capacity (will be parameterized when we integrate)
    ...
    def calculate_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_buses = sum(1 for v in vehicle_metrics.values() 
                            if self.vehicle_records[v].get("vehicle_type") == "Bus" 
                            and vehicle_metrics[v]["total_passengers"] > 0)
            active_minibuses = sum(1 for v in vehicle_metrics.values() 
                                if self.vehicle_records[v].get("vehicle_type") == "Minibus" 
                                and vehicle_metrics[v]["total_passengers"] > 0)

            # Then update the return dictionary to include:
            system_metrics = {
                "simulation_duration": self.simulation_duration,
                "total_vehicles": len(self.vehicle_records),
                "total_buses": total_buses,  # ADD THIS
                "total_minibuses": total_minibuses,  # ADD THIS
                "active_vehicles": active_vehicles,
                "active_buses": active_buses,  # ADD THIS
                "active_minibuses": active_minibuses,  # ADD THIS           
    ...
    def generate_report(self, output_file: Optional[str] = None) -> str:
        ...
        try:
            ...
            for vehicle_id in sorted(vehicle_metrics.keys()):
                v_metrics = vehicle_metrics[vehicle_id]
    ...
    def plot_occupancy_over_time(
        ...
        try:
            ...
            # Add capacity line if single vehicle
    ...
    def plot_minibus_vs_bus_comparison(self, output_file: str = "vehicle_comparison.png") -> None:
        """
        Compare performance metrics between buses and minibuses.
        
        Args:
            output_file: Path to save the plot
        
        Creates:
            - Bar chart comparing buses vs minibuses
            - Metrics: passengers served, avg occupancy, utilization rate
        """
        logger.info("Creating bus vs minibus comparison plot...")
        
        try:
            vehicle_metrics = self.calculate_vehicle_metrics()
            
            # Separate buses and minibuses
            bus_data = [v for vid, v in vehicle_metrics.items() 
                        if self.vehicle_records[vid].get("vehicle_type") == "Bus"]
            minibus_data = [v for vid, v in vehicle_metrics.items() 
                            if self.vehicle_records[vid].get("vehicle_type") == "Minibus"]
            
            if not minibus_data:
                logger.warning("No minibus data available for comparison plot")
                return
            
            # Calculate averages
            metrics = ['total_passengers', 'avg_occupancy', 'occupancy_rate']
            bus_avgs = [np.mean([v[m] for v in bus_data]) if bus_data else 0 
                        for m in metrics]
            minibus_avgs = [np.mean([v[m] for v in minibus_data]) if minibus_data else 0 
                            for m in metrics]
            
            # Create plot
            fig, axes = plt.subplots(1, 3, figsize=(15, 5))
            labels = ['Buses', 'Minibuses']
            colors = ['steelblue', 'coral']
            
            titles = ['Avg Passengers Served', 'Avg Occupancy', 'Avg Occupancy Rate (%)']
            for idx, (ax, title) in enumerate(zip(axes, titles)):
                values = [bus_avgs[idx], minibus_avgs[idx]]
                if idx == 2:  # Convert occupancy rate to percentage
        ...
        try:
            ...
            with open(vehicles_file, 'w', newline='', encoding='utf-8') as f:
                fieldnames = [
                    ...
                    "total_alightings", "stations_served"
                ]
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
