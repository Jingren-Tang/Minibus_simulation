...
# Save this as: verify_with_google_api.py
# Usage: python verify_with_google_api.py YOUR_API_KEY
...
for result in results:
    if result['found']:
        ...
        folium.CircleMarker(
            ...
            fillColor='green',
            fillOpacity=0.6,
            popup=f"Google: {station['name']}<br>{result['formatted_address']}"
        ).add_to(m)
...
m.save('/mnt/user-data/outputs/google_geocoding_comparison.html')
print("Map saved to: /mnt/user-data/outputs/google_geocoding_comparison.html")
