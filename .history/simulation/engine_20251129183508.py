...
class SimulationEngine:
    ...
    def initialize(self) -> None:
        ...
        self.network = TransitNetwork(
            ...
            travel_time_matrix=self.config["travel_time_matrix"],
            matrix_metadata=self.config["matrix_metadata"]
        )
    ...
    def _load_buses_from_schedule(self) -> Dict[str, Bus]:
        ...
        try:
            ...
            for bus_id, schedule_data in bus_schedules.items():
                ...
                schedule = [stop["arrival_time"] for stop in schedule_data["stops"]]
                ...
                bus = Bus(
                    ...
                    route=route,
                    schedule=schedule,
                    capacity=self.config.get("bus_capacity", 50),  # Default capacity
                ...
                logger.debug(
                    ...
                    f"{len(route)} stops, first departure at {schedule[0]}s"
                )
    ...
    def handle_bus_arrival(self, event: Event) -> None:
        ...
        try:
            ...
            logger.info(
                ...
                f"occupancy: {bus.current_occupancy}/{bus.capacity}"
            )
        ...
        for passenger in self.pending_requests[:]:  # Use slice to avoid modification during iteration
            if passenger.state == Passenger.WAITING:
                ...
                if passenger.check_timeout(self.current_time):
                    ...
                    station = self.network.get_station(passenger.origin)
                    if station:
                        station.remove_waiting_passenger(passenger)
        ...
        if abandoned_passengers:
            ...
            for pax in abandoned_passengers:
                logger.debug(
                    ...
                    f"waited {self.current_time - pax.request_time:.1f}s"
                )
    ...
    def finalize(self) -> None:
        ...
        arrived = sum(1 for p in self.all_passengers.values() if p.state == Passenger.ARRIVED)
        abandoned = sum(1 for p in self.all_passengers.values() if p.state == Passenger.ABANDONED)
        waiting = sum(1 for p in self.all_passengers.values() if p.state == Passenger.WAITING)
        onboard = sum(1 for p in self.all_passengers.values() if p.state == Passenger.ONBOARD)
