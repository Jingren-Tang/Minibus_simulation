...
def create_minimal_test_data():
    ...
    # Remove old mockdata if exists to ensure clean start
    if os.path.exists(mockdata_dir):
        import shutil
        shutil.rmtree(mockdata_dir)
        logger.info(f"Removed old mockdata directory")
    
    # Create mockdata directory
    os.makedirs(mockdata_dir, exist_ok=True)
    logger.info(f"Created mockdata directory: {mockdata_dir}")
    ...
    with open(metadata_file, "r") as f:
        ...
        logger.info(f"Verified metadata keys: {list(verify_metadata.keys())}")
        if "station_mapping" in verify_metadata:
            logger.info(f"✓ station_mapping field confirmed in metadata")
        else:
            logger.error(f"✗ station_mapping field MISSING from metadata!")
    ...
    try:
        ...
        for bus_id, bus in engine.buses.items():
            logger.info(
                ...
                f"first arrival at {list(bus.schedule.values())[0]}s"
            )
...
if __name__ == "__main__":
    # Always recreate test data to ensure it's correct
    logger.info("Recreating test data to ensure correct format...")
