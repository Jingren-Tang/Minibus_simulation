...
class SimulationEngine:
    ...
    def handle_optimize_call(self, event: Event) -> None:
        ...
        Steps:
            ...
            6. **[KEY FIX] Remove assigned passengers from pending_requests**
            7. Schedule arrival events for minibuses with new routes
            8. Schedule next optimizer call
            9. Record statistics
        ...
        try:
            ...
            # [NEW] Log pending_requests before optimization for debugging
            logger.debug(
                f"Pending passenger IDs before optimization: "
                f"{[p.passenger_id for p in self.pending_requests]}"
            )
            ...
            # [NEW] Log current route plans for each minibus
            for mb_state in minibus_states:
                if mb_state["route_plan"]:
                    passengers_in_plan = set()
                    for stop in mb_state["route_plan"]:
                        passengers_in_plan.update(stop["passenger_ids"])
                    logger.debug(
                        f"{mb_state['minibus_id']} current route: {passengers_in_plan}"
                    )
            ...
            # [KEY FIX] Track newly assigned passengers
            newly_assigned_passenger_ids = set()
            ...
            for minibus_id, route_plan in new_plans.items():
                ...
                try:
                    ...
                    # [KEY FIX] Collect all passenger IDs in this route plan
                    if len(route_plan) > 0:
                        for stop in route_plan:
                            if stop["action"] == "PICKUP":
                                for passenger_id in stop["passenger_ids"]:
                                    newly_assigned_passenger_ids.add(passenger_id)
                                    
                                    # Find the passenger and mark as assigned
                                    ...
                                    for passenger in self.pending_requests:
                                        if passenger.passenger_id == passenger_id:
                                            ...
                                            if passenger.assigned_vehicle_id is None:
                                                ...
                                                logger.warning(
                                                    f"⚠️  DUPLICATE ASSIGNMENT DETECTED: "
                                                    f"Passenger {passenger_id} already assigned to "
                                                    f"{passenger.assigned_vehicle_id}, "
                                                    f"but optimizer assigned to {minibus_id}"
                                                )
            ...
            # [KEY FIX] Remove assigned passengers from pending_requests
            passengers_removed = 0
            original_count = len(self.pending_requests)
            
            self.pending_requests = [
                p for p in self.pending_requests 
                if p.passenger_id not in newly_assigned_passenger_ids
            ]
            
            passengers_removed = original_count - len(self.pending_requests)
            ...
            logger.info(
                ...
                f"{passengers_removed} passengers removed from pending_requests"
            )
            
            # [NEW] Verify no duplicate assignments
            logger.debug(
                f"Pending passenger IDs after optimization: "
                f"{[p.passenger_id for p in self.pending_requests]}"
            )
            ...
            self.statistics.record_system_event(
                ...
                description=(
                    f"Optimizer called: {original_count} pending requests, "
                    f"{plans_updated} plans updated, {events_scheduled} events scheduled, "
                    f"{passengers_assigned} passengers assigned, "
                    f"{passengers_removed} removed from pending"
                ),
                current_time=self.current_time
            )
