...
class SimulationEngine:
    ...
    def handle_minibus_arrival(self, event: Event) -> None:
        ...
        # 🔍 检测重复调用
        check_key = f"{minibus_id}_{self.current_time}"
        if not hasattr(self, '_processed_arrivals'):
            self._processed_arrivals = set()
        
        if check_key in self._processed_arrivals:
            logger.error(
                f"🚨 handle_minibus_arrival 被重复调用！"
                f"{minibus_id} at time {self.current_time}"
            )
            return  # 直接返回，不处理
        
        self._processed_arrivals.add(check_key)
        minibus_id = event.data["minibus_id"]
