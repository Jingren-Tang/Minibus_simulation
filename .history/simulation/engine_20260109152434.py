...
class SimulationEngine:
    ...
    def _generate_passengers_from_od_matrix(self) -> None:
        """
        MODIFIED: Generate passengers with service mode assignment.
        ...
        total_passengers = 0
        minibus_passengers = 0  # NEW: Count minibus passengers
        bus_passengers = 0  # NEW: Count bus passengers
        ...
        for slot_idx in range(n_time_slots):
            ...
            for origin_id, dest_id, appear_time in passengers:
                ...
                # NEW: Determine service mode based on appear_time
                ...
                passenger = Passenger(
                    ...
                    service_mode=service_mode  # NEW: Pass service mode
                )
        ...
        # NEW: Log final statistics by mode
        logger.info(
    ...
    def handle_optimize_call(self, event: Event) -> None:
        """
        MODIFIED: Only pass minibus passengers to optimizer.
        """
        try:
            if not self.config.get("enable_minibus", False):
                logger.warning("OPTIMIZE_CALL event but minibus not enabled")
            ...
            # NEW: Log separate statistics for bus and minibus passengers
            logger.info(f"Optimizer call at {self._seconds_to_time_str(self.current_time)}")
            logger.info(
            ...
            new_plans = self.route_optimizer.optimize(
                pending_requests=self.pending_minibus_requests,  # MODIFIED: Only minibus passengers
                minibus_states=minibus_states,
                network=self.network,
                current_time=self.current_time
            )
            ...
            for minibus_id, route_plan in new_plans.items():
                ...
                # Skip if both plans are empty
                if len(current_plan) == 0 and len(route_plan) == 0:
                    logger.debug(f"{minibus_id} remains idle")
                    continue
                
                # Skip if already executing same route
                if (minibus.status == minibus.EN_ROUTE and 
                    ...
                    logger.info(
                        f"{minibus_id} already executing this route, skipping update"
                    )
                        ...
                        for stop in route_plan:
                            if stop["action"] == "PICKUP":
                                for pid in stop["passenger_ids"]:
                                    ...
                                    for pax in self.pending_minibus_requests:  # MODIFIED: Search in minibus list
                                        if pax.passenger_id == pid:
                                            if pax.assigned_vehicle_id is None:
                                                ...
                                                logger.warning(
                                                    ...
                                                    f"{pax.assigned_vehicle_id}, reassigning to {minibus_id}"
                                                )
                                            break
            ...
            # NEW: Remove assigned passengers from pending_minibus_requests
            original_count = len(self.pending_minibus_requests)
            
            self.pending_minibus_requests = [
                p for p in self.pending_minibus_requests
                if p.passenger_id not in newly_assigned_ids
            ]
            
            removed_count = original_count - len(self.pending_minibus_requests)
            
            # Also remove from old pending_requests for compatibility

            
            logger.info(
                f"Complete: {plans_updated} plans updated, {events_scheduled} events scheduled, "
                f"{removed_count} passengers assigned and removed from pending"
            )
            
            # Record statistics
            self.statistics.record_system_event(
                event_type="OPTIMIZER_CALL",
                description=f"Optimizer: {original_count} pending minibus passengers, {removed_count} assigned",
                current_time=self.current_time
            )
    ...
    def handle_periodic_sample(self, event: Event) -> None:
        """
        Handle periodic sampling event.
        
        Iterate through all active vehicles and record their current state to statistics.
        After sampling, schedule the next sampling event.
        
        Only samples vehicles that have been activated (started their service).
        Inactive vehicles (not yet departed or already completed) are skipped.
        ...
        try:
            ...
            # Sample all Buses (only those that are active)
            for bus_id, bus in self.buses.items():
                # Skip inactive buses: first departure time has not yet arrived
                # If next_arrival_time exists and is in the future, bus hasn't started yet
                if bus.next_arrival_time is not None and bus.next_arrival_time > self.current_time:
                    continue
                
                # Skip buses that have completed their entire route (optional)
                # If bus is at terminal and has visited all stations, it's done
                if bus.is_at_terminal() and bus.current_route_index >= len(bus.route):
                    continue
                
                # Get current location for this bus
                location = self._get_bus_location(bus)
                
                # Record the periodic state sample
            ...
            # Sample all Minibuses (only those that are active)
            for minibus_id, minibus in self.minibuses.items():
                # Skip inactive minibuses: next arrival time is in the future
                # This means the minibus hasn't started its route yet
                if minibus.next_arrival_time is not None and minibus.next_arrival_time > self.current_time:
                    continue
                
                # Skip idle minibuses that have never been used
                # If minibus is IDLE and has served 0 passengers, it's never been activated
                if minibus.status == minibus.IDLE and minibus.total_passengers_served == 0:
                    continue
            ...
            logger.debug(
                f"Periodic sampling complete: sampled {sampling_count} active vehicles "
                f"(out of {len(self.buses) + len(self.minibuses)} total) at {self.current_time}s"
            )
            
            # Schedule next periodic sampling event
            sampling_interval = self.config.get("sampling_interval", 30.0)
            next_sample_time = self.current_time + sampling_interval
            
            # Only schedule next sampling if it's within simulation duration
