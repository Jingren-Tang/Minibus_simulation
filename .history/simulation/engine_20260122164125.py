...
class SimulationEngine:
    ...
    def handle_minibus_arrival(self, event: Event) -> None:
            ...
            try:
                ...
                # If the optimizer updated the route, minibus.next_arrival_time changed,
                # but this old event is still in the queue. We must ignore it.
                
                if minibus.next_arrival_time is None:
                    # Minibus is IDLE or has no next stop, but an event fired. 
                    # This is definitely a stale event from a previous plan.
                    logger.debug(f"Ignoring stale event for {minibus_id} (bus is IDLE/No Plan)")
                    return
                ...
                # 4. Process arrival (Boarding/Alighting)
                ...
                # 5. Record Statistics (Arrival, Boarding, Alighting)
                # ... (Your existing statistics recording code matches logic here) ...
                self.statistics.record_vehicle_event(
                ...
                if len(boarded) > 0:
                    ...
                    # Remove boarded passengers from pending list
                    for passenger in boarded:
                        # Remove from minibus specific list
                ...
                if minibus.next_arrival_time is not None:
                    if minibus.next_station_id is None:
                        logger.error(f"{minibus_id}: next_arrival_time set but next_station_id is None!")
                    else:
                        self.add_event(Event(
                            time=minibus.next_arrival_time,
                            event_type=Event.MINIBUS_ARRIVAL,
                        ...
                        logger.debug(
                            f"Scheduled next arrival for {minibus_id} at {minibus.next_station_id} "
                            f"at {minibus.next_arrival_time:.1f}s"
                        )
                else:
                    logger.info(f"Minibus {minibus_id} completed plan, now IDLE")
                    self.statistics.record_system_event(
                        event_type="MINIBUS_IDLE",
                        description=f"{minibus_id} idle at {station.station_id}",
                        current_time=self.current_time
                    )
    ...
    def handle_optimize_call(self, event: Event) -> None:
        """
        Handle periodic optimizer call event.
        [FIXED VERSION] Safe scheduling relying on arrival event validation.
        """
        try:
            if not self.config.get("enable_minibus", False):
                logger.warning("OPTIMIZE_CALL event but minibus not enabled")
            ...
            for minibus_id, route_plan in new_plans.items():
                ...
                # Skip if both plans are empty (idle remains idle)
                if len(current_plan) == 0 and len(route_plan) == 0:
                    continue
                
                # Check if route is effectively the same to avoid unnecessary logging
                if (minibus.status == minibus.EN_ROUTE and 
                    ...
                    # Even if route is same, we assume the vehicle continues its current path.
                    # We DO NOT need to add a new event here because the existing one 
                    # in the queue is still valid (travel time hasn't changed).
                    # However, strictly applying the new plan (re-calculating time) is safer 
                    # to correct any drift, at the cost of one extra "stale" event check later.
                    pass 
                ...
                try:
                    ...
                    # 2. Track assigned passengers (for internal state & stats)
                    if len(route_plan) > 0:
                        for stop in route_plan:
                            if stop["action"] == "PICKUP":
                                for pid in stop["passenger_ids"]:
                                    ...
                                    # Mark passenger as assigned in the pending list
                                    for pax in self.pending_minibus_requests:
                                        if pax.passenger_id == pid:
                                            if pax.assigned_vehicle_id is None:
                                                ...
                                                logger.warning(
                                                    f"Reassigning {pid} from {pax.assigned_vehicle_id} to {minibus_id}"
                                                )
                                                pax.assigned_vehicle_id = minibus_id
                                            break
                    
                    # 3. Schedule next arrival event
            ...
            logger.info(
                ...
                f"{removed_count} passengers assigned"
            )
