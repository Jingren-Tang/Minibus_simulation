...
class SimulationEngine:
    ...
    def initialize(self) -> None:
        ...
        if self.config.get("enable_minibus", False):
            ...
            optimizer_interval = self.config.get("optimizer_call_interval", 300.0)
            self.add_event(Event(
    ...
    def finalize(self) -> None:
        ...
        if len(self.minibuses) > 0:
            ...
            total_served = sum(len(mb.passengers) for mb in self.minibuses.values())
            logger.info(f"    Total passengers served by minibuses: {total_served}")
        ...
        if self.statistics is not None:
            ...
            self.statistics.create_dashboard(
    ...
    def _load_minibuses_from_config(self) -> Dict[str, 'Minibus']:
        ...
        Configuration format:
            {
                "num_minibuses": int,  # or "minibus_fleet_size"
        ...
        try:
            # Get minibus count from config (support both key names)
            num_minibuses = self.config.get(
                "num_minibuses", 
                self.config.get("minibus_fleet_size", 0)
            )
    ...
    def handle_optimize_call(self, event: Event) -> None:
        ...
        Steps:
            ...
            5. Schedule arrival events for minibuses with new routes
            6. Schedule next optimizer call
            7. Record statistics
        ...
        try:
            ...
            for minibus_id, route_plan in new_plans.items():
                ...
                if len(route_plan) > 0:
                    ...
                    logger.debug(f"{minibus_id} received empty route plan (remain idle)")
