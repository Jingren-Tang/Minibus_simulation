...
class SimulationEngine:
    ...
    def _requeue_lost_waiting(self):
        pending_ids = set(self.pending_minibus_requests.keys()) | set(self.pending_bus_requests.keys())

        for p in self.all_passengers.values():
            if p.status != "WAITING":
                continue
            if p.passenger_id in pending_ids:
                continue
            if getattr(p, "assigned_vehicle", None):  # 已经分配了就别动
                continue

            # 按 service_mode 放回对应 pending
            if p.service_mode == "MINIBUS":
                self.pending_minibus_requests[p.passenger_id] = p
            else:
                self.pending_bus_requests[p.passenger_id] = p
            logger.error(f"Re-queued LOST WAITING {p.passenger_id} mode={p.service_mode}")


    def _ensure_onboard_dropoffs(self, minibus, route_plan):
        # Exit early if there are no passengers on board
        if not getattr(minibus, "passengers", None):
            return route_plan

        # Index existing drop-offs: (station_id -> set of passenger_ids)
        dropoff = {}
        for stop in route_plan:
            if stop.get("action") == "DROPOFF":
                dropoff.setdefault(stop["station_id"], set()).update(stop.get("passenger_ids", []))

        # All onboard passengers MUST have a corresponding drop-off action in the plan
        for p in minibus.passengers:
            dest = p.destination_station_id
            pid = p.passenger_id
            
            # Skip if the passenger is already accounted for at their destination
            if pid in dropoff.get(dest, set()):
                continue

            # If a drop-off stop already exists for this station, merge the passenger into it
            merged = False
            for stop in route_plan:
                if stop.get("action") == "DROPOFF" and stop["station_id"] == dest:
                    stop["passenger_ids"] = list(set(stop.get("passenger_ids", []) + [pid]))
                    merged = True
                    break

            # Otherwise, append a new drop-off stop to the end of the route 
            # (This is the most conservative approach to ensure no passenger is "stranded" on board)
            if not merged:
                route_plan.append({"station_id": dest, "action": "DROPOFF", "passenger_ids": [pid]})
