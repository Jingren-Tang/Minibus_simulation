...
def _convert_to_vehicle_objects(minibuses: List[Dict]) -> List[Dict]:
    ...
    🔧 FIXED: Only include future stops, not completed ones
    """
    vehicles = []
    ...
    for mb in minibuses:
        ...
        # 🔧 NEW: Only process stops that haven't been completed yet
        for stop in mb["current_route_plan"]:
            station = stop["station_id"]
            action = stop["action"]
            passenger_ids = stop["passenger_ids"]
            
            # Skip empty actions (already processed)
            if not passenger_ids:
                continue
            
            # For DROPOFF: verify passengers are actually onboard
            if action == "DROPOFF":
                valid_passengers = [
                    p for p in passenger_ids 
                    if p in mb["passengers_onboard"]
                ]
                if not valid_passengers:
                    logger.debug(
                        f"Skipping completed DROPOFF at {station}: "
                        f"{passenger_ids} not onboard"
                    )
                    continue
                passenger_ids = valid_passengers
            
            # Add to route (avoid duplicates)
            ...
            if action == "PICKUP":
                ...
                tracker[station]["dropoff"].extend(passenger_ids)
