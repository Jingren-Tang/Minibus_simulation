...
def _clean_route(vehicle: Dict) -> Dict:
    ...
    FIXED: Also removes consecutive duplicate stations from route.
    """
    route = vehicle["route"]
    tracker = vehicle["tracker"]
    
    cleaned_route = []
    seen_stations = set()  # ✅ Track seen stations
    
    for station in route:
        # Skip if:
        # 1. Already seen (duplicate), OR
        # 2. No pickup/dropoff at this station
