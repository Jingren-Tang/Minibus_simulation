...
def _try_insert_request(
    ...
    """
    Try to insert request with STATION REUSE support (Phase 2) - FIXED VERSION
    
    FIXES:
    - Case 1: Added duplicate check before adding passengers to tracker
    - Added detailed logging for debugging
    
    Handles 4 cases:
    1. Both origin and destination exist → Reuse both (FIXED)
        ...
        for o_pos in origin_positions:
            for d_pos in dest_positions:
                if o_pos < d_pos:
                    ...
                    # 🔧 FIX: Check for duplicates before adding passenger
                    if passenger_id in candidate_tracker[origin]["pickup"]:
                        logger.warning(
                            f"Case 1: Passenger {passenger_id} already in pickup list at {origin}, skipping this combination"
                        )
                        continue
                    
                    if passenger_id in candidate_tracker[destination]["dropoff"]:
                        logger.warning(
                            f"Case 1: Passenger {passenger_id} already in dropoff list at {destination}, skipping this combination"
                        )
                        continue
                    
                    # Add passenger (now safe - no duplicates)
                    candidate_tracker[origin]["pickup"].append(passenger_id)
                    candidate_tracker[destination]["dropoff"].append(passenger_id)
                    
                    logger.debug(
                        f"Case 1 attempt: {passenger_id} at o_pos={o_pos}, d_pos={d_pos}, "
                        f"pickup count at {origin}: {len(candidate_tracker[origin]['pickup'])}, "
                        f"dropoff count at {destination}: {len(candidate_tracker[destination]['dropoff'])}"
                    )
                    ...
                    if _check_capacity_feasible(vehicle, candidate_route, candidate_tracker):
                        ...
                        if cost < min_cost:
                            ...
                            best_tracker = candidate_tracker
                            logger.debug(
                                f"Case 1: Found feasible solution with cost={cost:.2f}"
                            )
                    else:
                        logger.debug(
                            f"Case 1: Capacity check failed for o_pos={o_pos}, d_pos={d_pos}"
                        )
...
def _check_capacity_feasible(
    ...
    ENHANCED: Added detailed logging for debugging negative occupancy issues.
    """
    capacity = vehicle["capacity"]
    occupancy = vehicle["current_occupancy"]
    
    logger.debug(
        f"Checking capacity for {vehicle['minibus_id']}: "
        f"initial occupancy={occupancy}/{capacity}"
    )
    ...
    for station in candidate_route:
        if station in candidate_tracker:
            ...
            logger.debug(
                f"  Station {station}: DROPOFF {len(dropoff_passengers)} passengers "
                f"(IDs: {dropoff_passengers}), occupancy now: {occupancy}"
            )
            ...
            logger.debug(
                f"  Station {station}: PICKUP {len(pickup_passengers)} passengers "
                f"(IDs: {pickup_passengers}), occupancy now: {occupancy}"
            )
            
            # 3. Check capacity constraint
            if occupancy > capacity:
                logger.debug(
                    f"  ❌ Capacity exceeded at {station}: {occupancy} > {capacity}"
                )
                return False
            
            # 4. Sanity check - THIS IS WHERE THE BUG SHOWS UP
            if occupancy < 0:
                logger.error(
                    f"  ❌ NEGATIVE OCCUPANCY at {station}: {occupancy}. "
                    f"Dropoff list: {dropoff_passengers}, "
                    f"Pickup list: {pickup_passengers}. "
                    f"This indicates duplicate dropoffs or missing passengers!"
                )
                return False
    
    logger.debug(f"  ✅ Capacity check passed, final occupancy: {occupancy}/{capacity}")
    return True
