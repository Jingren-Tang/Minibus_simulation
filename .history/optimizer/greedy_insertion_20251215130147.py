...
if __name__ == "__main__":
    ...
    for minibus_id, route_plan in output2.items():
        ...
        if not route_plan:
            ...
            # 🔧 FIX: Simulate occupancy correctly
            occupancy = 1  # Start with 1 existing passenger (P_existing)
            ...
            for i, stop in enumerate(route_plan):
                ...
                if action == 'DROPOFF':
                    ...
                    print(f"  {i+1}. {station}: DROPOFF {passengers} (occupancy: {occupancy}/3)")
                
                # Process PICKUP
                elif action == 'PICKUP':
                    occupancy += len(passengers)
                    print(f"  {i+1}. {station}: PICKUP {passengers} (occupancy: {occupancy}/3)")
