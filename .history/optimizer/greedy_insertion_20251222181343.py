...
def _convert_to_vehicle_objects(minibuses: List[Dict]) -> List[Dict]:
    ...
    🔧 CRITICAL FIX: Preserve ALL existing route plans for onboard passengers
    """
    vehicles = []
    
    for mb in minibuses:
        # 🔧 KEY FIX: If vehicle has passengers onboard, keep the ENTIRE route plan
        # Otherwise optimizer will think the vehicle is full forever!
        
        if mb["current_occupancy"] > 0 and len(mb["passengers_onboard"]) > 0:
            # Vehicle has passengers - keep existing route plan as-is
            logger.info(
                f"{mb['minibus_id']}: Has {mb['current_occupancy']} passengers onboard, "
                f"preserving existing route plan ({len(mb['current_route_plan'])} stops)"
            )
            ...
            for stop in mb["current_route_plan"]:
                ...
                if stop["action"] == "PICKUP":
                    ...
                    # 🔧 Keep ALL dropoffs - don't filter!
                    tracker[station]["dropoff"].extend(stop["passenger_ids"])
        
        else:
            # Vehicle is empty - start fresh
            logger.info(f"{mb['minibus_id']}: Empty vehicle, starting with blank route")
            current_route = []
            tracker = {}
