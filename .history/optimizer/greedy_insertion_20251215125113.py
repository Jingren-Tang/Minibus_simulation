...
def greedy_insert_optimize(input_data: dict) -> Dict[str, List[Dict]]:
    ...
    for request in pending_requests:
        ...
        if best_vehicle is not None:
            ...
            best_vehicle["tracker"] = best_tracker  # 🆕 关键修复
...
def _try_insert_request(
    ...
    """
    Try to insert request with STATION REUSE logic
    """
    origin = request["origin"]
    destination = request["destination"]
    passenger_id = request["passenger_id"]
    ...
    # 🆕 Check if origin/destination already exist in route
    ...
    # Case 1: Both origin and destination already exist
    if origin_positions and dest_positions:
        # Try all combinations where origin_pos < dest_pos
        for o_pos in origin_positions:
            for d_pos in dest_positions:
                if o_pos < d_pos:
                    # Reuse existing stations
                    candidate_route = current_route.copy()
                    candidate_tracker = _build_tracker_with_reuse(
                        current_tracker, 
                        passenger_id, 
                        origin, 
                        destination
    ...
    # Case 3: Only destination exists, need to insert origin
        ...
        for d_pos in dest_positions:
            # Insert origin before destination
            for o_insert_pos in range(0, d_pos + 1):
                candidate_route = current_route.copy()
                candidate_route.insert(o_insert_pos, origin)
                
                # Note: d_pos shifts by 1 after insertion
                adjusted_d_pos = d_pos + 1
                ...
                candidate_tracker = _build_tracker_mixed(
                    ...
                    origin,
                    destination,
                    origin_insert_pos=o_insert_pos,
                    dest_reused=True
                )
    ...
    # Case 4: Neither exists, need to insert both (original logic)
    else:
...
def _build_tracker_with_reuse(current_tracker, passenger_id, origin, destination):
    """
    Build tracker when reusing existing stations
    """
    tracker = {}
    for station, actions in current_tracker.items():
        tracker[station] = {
    ...
    if origin not in tracker:
        tracker[origin] = {"pickup": [], "dropoff": []}
    tracker[origin]["pickup"].append(passenger_id)
    
    if destination not in tracker:
        tracker[destination] = {"pickup": [], "dropoff": []}
    tracker[destination]["dropoff"].append(passenger_id)
...
def _check_capacity_feasible(
    ...
    """
    Check capacity constraint (FIXED VERSION)
    
    Uses tracker to accurately simulate occupancy changes.
    """
