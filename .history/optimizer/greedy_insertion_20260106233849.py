...
def _convert_to_vehicle_objects(minibuses: List[Dict]) -> List[Dict]:
    ...
    CRITICAL FIX: Use passengers_onboard length as authoritative occupancy source,
    not the provided current_occupancy which may be inconsistent.
    """
    vehicles = []
    ...
    for mb in minibuses:
        ...
        # ✅ FIX: Use actual passengers_onboard count, not reported occupancy
        ...
        if actual_occupancy != reported_occupancy:
            logger.warning(
                ...
                f"Actual (passengers_onboard): {actual_occupancy}. "
        ...
        # =====================================================================
        # CRITICAL VALIDATION: Check tracker consistency with passengers_onboard
        # =====================================================================
        onboard_set = set(passengers_onboard)
        
        # All passengers scheduled for PICKUP should NOT be onboard yet
        future_pickups = set()
        for actions in tracker.values():
            future_pickups.update(actions["pickup"])
        
        conflict_pickup = future_pickups & onboard_set
        if conflict_pickup:
            logger.error(
                f"{minibus_id}: Passengers {conflict_pickup} are BOTH "
                f"onboard AND scheduled for future pickup! "
                f"Route plan contains already-completed pickup stops. "
                f"Removing them from future pickups."
            )
            # Fix: remove these passengers from tracker pickups
            for station, actions in tracker.items():
                actions["pickup"] = [p for p in actions["pickup"] if p not in conflict_pickup]
        
        # All passengers scheduled for DROPOFF should be onboard
        future_dropoffs = set()
        for actions in tracker.values():
            future_dropoffs.update(actions["dropoff"])
        
        missing_dropoff = future_dropoffs - onboard_set
        if missing_dropoff:
            logger.error(
                f"{minibus_id}: Passengers {missing_dropoff} scheduled for "
                f"dropoff but NOT onboard! "
                f"This indicates assigned-but-not-yet-picked-up passengers "
                f"or data inconsistency. Removing them from future dropoffs."
            )
            # Fix: remove these passengers from tracker dropoffs
            for station, actions in tracker.items():
                actions["dropoff"] = [p for p in actions["dropoff"] if p not in missing_dropoff]
        
        # =====================================================================
        # Final validation: simulate route execution
        # =====================================================================
        ...
        for station in current_route:
            if station in tracker:
                ...
                if test_occupancy < 0:
                    logger.error(
                        f"{minibus_id}: NEGATIVE occupancy {test_occupancy} "
                        f"at {station} after {dropoff_count} dropoffs! "
                        f"Initial occupancy: {actual_occupancy}, "
                        f"passengers_onboard: {passengers_onboard}, "
                        f"tracker: {tracker[station]}"
                    )
                    # Emergency fix: cap at 0
                    test_occupancy = 0
                
                test_occupancy += pickup_count
                
                if test_occupancy > mb["capacity"]:
                    logger.error(
                        f"{minibus_id}: OVER capacity {test_occupancy}/"
                        f"{mb['capacity']} at {station}"
                    )
        
        # =====================================================================
        # Create vehicle object with corrected occupancy
        # =====================================================================
        vehicle = {
        ...
        logger.debug(
            f"{minibus_id}: Converted to vehicle object - "
            f"occupancy={actual_occupancy}, route_length={len(current_route)}, "
