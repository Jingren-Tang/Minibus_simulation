...
def greedy_insert_optimize(input_data: dict) -> Dict[str, List[Dict]]:
    """Main optimization function with detailed debugging"""
    ...
    # 🔧 DEBUG: Print initial vehicle states
    logger.info("\n=== Initial Vehicle States ===")
    for vehicle in vehicles:
        logger.info(f"{vehicle['minibus_id']}:")
        logger.info(f"  Occupancy: {vehicle['current_occupancy']}/{vehicle['capacity']}")
        logger.info(f"  Route: {vehicle['route']}")
        logger.info(f"  Tracker: {vehicle['tracker']}")
    ...
    for idx, request in enumerate(pending_requests):
        ...
        logger.info(f"\n=== Processing passenger {idx+1}/{len(pending_requests)}: {passenger_id} ===")
        ...
        # 🔧 DEBUG: Track attempts for each vehicle
        for vehicle in vehicles:
            logger.debug(f"\nTrying {vehicle['minibus_id']}:")
            logger.debug(f"  Current route: {vehicle['route']}")
            logger.debug(f"  Current occupancy: {vehicle['current_occupancy']}/{vehicle['capacity']}")
            ...
            if candidate_route is not None:
                logger.debug(f"  ✓ Feasible! Cost: {cost:.2f}")
                logger.debug(f"    New route: {candidate_route}")
                if cost < best_cost:
                ...
                logger.debug(f"  ✗ Not feasible (cost: inf)")
        ...
        if best_vehicle is not None:
            ...
            logger.info(
                f"✓ Assigned {passenger_id} to {best_vehicle['minibus_id']}, cost={best_cost:.2f}"
            )
        else:
            logger.warning(
            ...
            # 🔧 DEBUG: Why not?
            logger.warning(f"  All {len(vehicles)} vehicles rejected this passenger")
            for vehicle in vehicles:
                logger.warning(
                    f"    {vehicle['minibus_id']}: occupancy={vehicle['current_occupancy']}/{vehicle['capacity']}, "
                    f"route_length={len(vehicle['route'])}"
                )
    
    output = _convert_to_output_format(vehicles)
    
    logger.info(
        f"\n=== Optimization complete: assigned {len(assigned_passengers)}/"
...
def _try_insert_request(
    ...
    """
    Try to insert request with detailed failure logging
    """
    origin = request["origin"]
    destination = request["destination"]
    passenger_id = request["passenger_id"]
    ...
    # 🔧 DEBUG: Track how many insertion attempts were made
    total_attempts = 0
    feasible_attempts = 0
    ...
    # === CASE 1: Both stations exist ===
    if origin_positions and dest_positions:
        logger.debug(f"    Case 1: Both stations exist")
    ...
    # === CASE 2: Only origin exists ===
        ...
        logger.debug(f"    Case 2: Only origin exists")
        for o_pos in origin_positions:
    ...
    # === CASE 3: Only destination exists ===
        ...
        logger.debug(f"    Case 3: Only destination exists")
        for d_pos in dest_positions:
    ...
    # === CASE 4: Neither exists ===
        ...
        logger.debug(f"    Case 4: Neither station exists")
    ...
    logger.debug(f"    Tried {total_attempts} insertions, {feasible_attempts} were feasible")
    if best_route is None:
        logger.debug(f"    ✗ All insertions failed capacity check")
...
def _check_capacity_feasible(
    ...
    Added defensive checks for tracker consistency.
    """
    capacity = vehicle["capacity"]
    occupancy = vehicle["current_occupancy"]
    passengers_onboard = set(vehicle["passengers_onboard"])  # 🔧 NEW: Track who's actually onboard
    
    for station in candidate_route:
        if station in candidate_tracker:
            # 🔧 NEW: Validate dropoffs before processing
            dropoff_passengers = candidate_tracker[station]["dropoff"]
            
            # Filter out passengers who aren't actually onboard
            valid_dropoffs = [p for p in dropoff_passengers if p in passengers_onboard]
            invalid_dropoffs = [p for p in dropoff_passengers if p not in passengers_onboard]
            
            if invalid_dropoffs:
                logger.warning(
                    f"Tracker inconsistency at {station}: "
                    f"Trying to dropoff {invalid_dropoffs} who aren't onboard. "
                    f"Onboard: {passengers_onboard}"
                )
                # This is an invalid route - reject it
                return False
            
            # 1. Passengers DROPOFF (get off)
            occupancy -= len(valid_dropoffs)
            for p in valid_dropoffs:
            ...
            if occupancy < 0:
                logger.error(
                    f"CRITICAL: Negative occupancy at {station}: {occupancy}. "
...
def _convert_to_vehicle_objects(minibuses: List[Dict]) -> List[Dict]:
    """
    🔧 ROBUST VERSION: Rebuild tracker from ground truth
    """
    vehicles = []
    ...
    for mb in minibuses:
        ...
        # Step 1: Extract route structure (deduplicated)
        for stop in mb["current_route_plan"]:
            station = stop["station_id"]
            if not current_route or current_route[-1] != station:
                current_route.append(station)
        
        # Step 2: Rebuild tracker from current_route_plan
        for stop in mb["current_route_plan"]:
            station = stop["station_id"]
            action = stop["action"]
            passenger_ids = stop["passenger_ids"]
            ...
            if action == "PICKUP":
                # PICKUP: Keep all (they're future passengers)
                tracker[station]["pickup"].extend(passenger_ids)
            
            elif action == "DROPOFF":
                # DROPOFF: Only keep if passenger is currently onboard
                for p in passenger_ids:
                    if p in mb["passengers_onboard"]:
                        tracker[station]["dropoff"].append(p)
                    else:
                        logger.debug(
                            f"{mb['minibus_id']}: Skipping DROPOFF for {p} "
                            f"at {station} (already completed or not onboard)"
                        )
        
        # Step 3: Clean up empty stations
        tracker = {
            station: actions 
            for station, actions in tracker.items() 
            if actions["pickup"] or actions["dropoff"]
        }
        
        # Step 4: Rebuild route to only include stations with actions
        current_route = [s for s in current_route if s in tracker]
