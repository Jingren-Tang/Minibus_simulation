...
def _convert_to_vehicle_objects(minibuses: List[Dict]) -> List[Dict]:
    ...
    CRITICAL FIX: Properly handle passengers already onboard whose pickup
    is not in the remaining route_plan.
    """
    vehicles = []
    ...
    for mb in minibuses:
        ...
        # Use actual passengers_onboard count
        actual_occupancy = len(passengers_onboard)
        reported_occupancy = mb["current_occupancy"]
        ...
        # =====================================================================
        # Build route as simple station list WITHOUT duplicates
        # =====================================================================
        current_route = []
        seen_stations = set()
        ...
        # =====================================================================
        # Build tracking info: aggregate all actions for each station
        # =====================================================================
        tracker = {}
        for stop in current_route_plan:
            station = stop["station_id"]
        ...
        # ✅ FIX: Distinguish between different types of conflicts
        
        # Type 1: Passengers scheduled for PICKUP but already onboard
        # This is a real error - pickup was already completed but still in plan
        conflict_pickup = all_pickups & onboard_set
        if conflict_pickup:
            logger.error(
                f"  ❌ ERROR: Passengers {conflict_pickup} are BOTH "
                f"onboard AND scheduled for future pickup! "
                f"Their pickup was already completed but not removed from route_plan."
            )
        
        # Type 2: Passengers scheduled for DROPOFF but not in future pickups
        # Split into two cases:
        passengers_to_dropoff_no_pickup = all_dropoffs - all_pickups
        
        # Case A: They are currently onboard - this is NORMAL
        # (These passengers were picked up earlier, now just need to be dropped off)
        already_onboard_dropoffs = passengers_to_dropoff_no_pickup & onboard_set
        
        # Case B: They are NOT onboard - this is an ERROR
        # (Scheduled for dropoff but never picked up and not currently onboard)
        invalid_dropoffs = passengers_to_dropoff_no_pickup - onboard_set
        ...
        if invalid_dropoffs:
            logger.error(
                f"  ❌ ERROR: Passengers {invalid_dropoffs} scheduled for "
                f"dropoff but NOT onboard and no pickup scheduled! "
                f"This indicates data inconsistency."
            )
        
        # Type 3: Passengers onboard but not scheduled for dropoff
        orphan_passengers = onboard_set - all_dropoffs
        if orphan_passengers:
            logger.warning(
                f"  ⚠️  WARNING: Passengers {orphan_passengers} are onboard "
                f"but NOT scheduled for dropoff in route_plan! "
                f"They may be stuck on the vehicle."
            )
