...
def greedy_insert_optimize(input_data: dict) -> Dict[str, List[Dict]]:
    ...
    for request in pending_requests:
        ...
        for vehicle in vehicles:
            candidate_route, cost = _try_insert_request(vehicle, request, input_data)
...
def _try_insert_request(vehicle, request, input_data):
    origin = request["origin"]
    destination = request["destination"]
    passenger_id = request["passenger_id"]
    current_route = vehicle["route"]
    ...
    for i in range(len(current_route) + 1):
        for j in range(i + 1, len(current_route) + 2):
            candidate_route = current_route.copy()
            candidate_route.insert(i, (origin, "PICKUP", passenger_id))
            candidate_route.insert(j, (destination, "DROPOFF", passenger_id))
            
            if _check_capacity_feasible(vehicle, candidate_route):
                cost = _compute_route_cost(candidate_route, input_data)
                if cost < min_cost:
                    min_cost = cost
                    best_route = candidate_route
...
def _check_capacity_feasible(
    ...
    for station_id, action, passenger_id in candidate_route:
        if action == "DROPOFF":
            occupancy -= 1
        elif action == "PICKUP":
            occupancy += 1
    ...
    for mb in minibuses:
        ...
        for stop in mb["current_route_plan"]:
            ...
            for passenger_id in stop["passenger_ids"]:
                route.append((station, action, passenger_id))
...
def _compute_route_cost(route: List[str], input_data: Dict) -> float:
    ...
    station_sequence = []
    for station_id, action, passenger_id in route:
    ...
    for i in range(len(station_sequence) - 1):
        travel_time = get_travel_time(station_sequence[i], station_sequence[i+1], arrival_time)
...
def _convert_to_output_format(vehicles: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for vehicle in vehicles:
        ...
        while i < len(route):
            ...
            current_action = route[i][1]
            passenger_ids = []
            
            while i < len(route) and route[i][0] == current_station and route[i][1] == current_action:
                passenger_ids.append(route[i][2])
                i += 1
