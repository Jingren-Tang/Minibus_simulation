    ...
    if origin_positions and dest_positions:
        ...
        for o_pos in origin_positions:
            for d_pos in dest_positions:
                ...
                if not _check_capacity_feasible(vehicle, candidate_route, candidate_tracker):
                    continue
                
                # NEW: Check waiting time
                if not _check_waiting_time_feasible(
                    vehicle=vehicle,
                ...
                # NEW: Check travel quality
                if not _check_travel_quality_feasible(
                    vehicle=vehicle,
                    candidate_route=candidate_route,
                    candidate_tracker=candidate_tracker,
                    input_data=input_data
    ...
    # === CASE 4: Neither exists, insert both (original logic) ===
        ...
        for i in range(len(current_route) + 1):
            for j in range(i + 1, len(current_route) + 2):
                ...
                # NEW: Check waiting time constraint (for new passenger only)
                if not _check_waiting_time_feasible(
                    vehicle=vehicle,
                    candidate_route=candidate_route,
                    candidate_tracker=candidate_tracker,
                ...
                # NEW: Check travel quality constraints (for all passengers)
                if not _check_travel_quality_feasible(
                    vehicle=vehicle,
                    candidate_route=candidate_route,
                    candidate_tracker=candidate_tracker,
                ...
                # If all checks pass, compute cost
                cost = _compute_route_cost(candidate_route, input_data)
                if cost < min_cost:
                    min_cost = cost
                    best_route = candidate_route
...
def _check_waiting_time_feasible(
    vehicle: Dict,
    candidate_route: List[str],
    candidate_tracker: Dict,
    passenger_id: str,
    ...
    """
    Check if the new passenger's waiting time satisfies the constraint.
    
    Waiting time is defined as:
        waiting_time = pickup_time - appear_time
    
    Args:
        vehicle: Vehicle state dictionary
        candidate_route: Proposed route (list of station IDs)
        candidate_tracker: Pickup/dropoff tracker for the route
        passenger_id: ID of the passenger being inserted
        origin_station: Origin station of the passenger
        appear_time: Time when passenger appeared in the system
        input_data: Input data containing constraints and travel time function
    
    Returns:
        bool: True if waiting time constraint is satisfied, False otherwise
    """
    max_waiting_time = input_data.get("max_waiting_time", 600.0)
    current_time = input_data["current_time"]
    get_travel_time = input_data["get_travel_time"]
    
    # 1. Find the pickup position of the new passenger
    pickup_idx = None
    for i, station in enumerate(candidate_route):
        if (station == origin_station and 
            station in candidate_tracker and
            passenger_id in candidate_tracker[station]["pickup"]):
            pickup_idx = i
            break
    
    if pickup_idx is None:
        logger.warning(
            f"Cannot find pickup position for passenger {passenger_id} "
            f"at station {origin_station}"
        )
        return False
    
    # 2. Calculate cumulative time from current location to pickup station
    cumulative_time = current_time
    current_location = vehicle["current_location"]
    
    for i in range(pickup_idx + 1):
        next_station = candidate_route[i]
        travel_time = get_travel_time(current_location, next_station, cumulative_time)
        cumulative_time += travel_time
        current_location = next_station
    
    pickup_time = cumulative_time
    ...
    if waiting_time > max_waiting_time:
        logger.debug(
            f"Passenger {passenger_id}: waiting_time={waiting_time:.1f}s > "
            f"max_waiting_time={max_waiting_time:.1f}s (REJECTED)"
        )
        return False
    
    logger.debug(
        f"Passenger {passenger_id}: waiting_time={waiting_time:.1f}s <= "
        f"max_waiting_time={max_waiting_time:.1f}s (OK)"
    )
    return True


def _check_travel_quality_feasible(
    vehicle: Dict,
    candidate_route: List[str],
    candidate_tracker: Dict,
    input_data: Dict
    ...
    """
    Check if all passengers' travel quality satisfies constraints.
    
    This function checks TWO constraints for ALL passengers (both onboard and new):
    1. Detour time constraint: actual_travel_time - direct_travel_time <= max_detour_time
    2. Travel time ratio constraint: actual_travel_time / direct_travel_time <= max_ratio
    
    Where:
        - actual_travel_time: Time traveling along the route from pickup to dropoff
        - direct_travel_time: Time of direct travel from origin to destination
    ...
    Returns:
        bool: True if all passengers satisfy travel quality constraints, False otherwise
    """
    max_detour_time = input_data.get("max_detour_time", 300.0)
    max_travel_time_ratio = input_data.get("max_travel_time_ratio", 1.5)
    current_time = input_data["current_time"]
    get_travel_time = input_data["get_travel_time"]
    
    # 1. Collect all passengers to check
    passengers_to_check = set()
    
    # 1.1 Passengers already onboard the vehicle
    passengers_to_check.update(vehicle["passengers_onboard"])
    
    # 1.2 Passengers in the route plan (to be picked up)
    for station, actions in candidate_tracker.items():
        passengers_to_check.update(actions["pickup"])
    
    # 2. For each passenger, check their travel quality
    for passenger_id in passengers_to_check:
        # Find pickup and dropoff positions for this passenger
        pickup_station = None
        ...
        for i, station in enumerate(candidate_route):
            if station in candidate_tracker:
                if passenger_id in candidate_tracker[station]["pickup"]:
                    pickup_station = station
                    pickup_idx = i
                if passenger_id in candidate_tracker[station]["dropoff"]:
                    dropoff_station = station
                    dropoff_idx = i
        
        # If cannot find complete pickup-dropoff pair, skip this passenger
        # (This can happen for passengers already onboard whose dropoff is not in route yet)
        if pickup_station is None or dropoff_station is None:
            logger.debug(
                f"Passenger {passenger_id}: incomplete journey in route, "
                f"skipping travel quality check"
            )
            continue
        ...
        if pickup_idx >= dropoff_idx:
            logger.warning(
                f"Passenger {passenger_id}: pickup_idx={pickup_idx} >= "
                f"dropoff_idx={dropoff_idx}, invalid route"
            )
            return False
        
        # 3. Calculate actual_travel_time (from pickup to dropoff along the route)
        cumulative_time = current_time
        current_location = vehicle["current_location"]
        
        # 3.1 First, travel from current location to pickup station
        for i in range(pickup_idx + 1):
            next_station = candidate_route[i]
            travel_time = get_travel_time(current_location, next_station, cumulative_time)
            cumulative_time += travel_time
        ...
        # 3.2 Then, travel from pickup to dropoff station
        for i in range(pickup_idx + 1, dropoff_idx + 1):
            next_station = candidate_route[i]
            travel_time = get_travel_time(current_location, next_station, cumulative_time)
            cumulative_time += travel_time
            current_location = next_station
        
        dropoff_time = cumulative_time
        actual_travel_time = dropoff_time - pickup_time
        
        # 4. Calculate direct_travel_time (direct journey from origin to destination)
        direct_travel_time = get_travel_time(pickup_station, dropoff_station, pickup_time)
        
        # 5. Calculate quality metrics
        detour_time = actual_travel_time - direct_travel_time
        travel_time_ratio = (
            actual_travel_time / direct_travel_time 
            if direct_travel_time > 0 
            else 0
        )
        
        # 6. Check detour time constraint
        if detour_time > max_detour_time:
            logger.debug(
                f"Passenger {passenger_id}: detour_time={detour_time:.1f}s > "
                f"max_detour_time={max_detour_time:.1f}s (REJECTED)"
            )
            return False
        
        # 7. Check travel time ratio constraint
        if travel_time_ratio > max_travel_time_ratio:
            logger.debug(
                f"Passenger {passenger_id}: travel_time_ratio={travel_time_ratio:.2f} > "
                f"max_travel_time_ratio={max_travel_time_ratio:.2f} (REJECTED)"
            )
            return False
        
        logger.debug(
            f"Passenger {passenger_id}: detour={detour_time:.1f}s, "
            f"ratio={travel_time_ratio:.2f} (OK)"
        )
