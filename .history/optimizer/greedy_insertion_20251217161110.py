...
def _try_insert_request(
    ...
    """
    Try to insert request with STATION REUSE support + Quality Constraints (Phase 3)
    
    Added constraints:
    1. max_detour_time: Maximum additional travel time vs direct route
    2. max_waiting_time: Maximum time passenger waits before pickup
    3. max_travel_time_ratio: Maximum ratio of actual vs direct travel time
    ...
    destination = request["destination"]
    passenger_id = request["passenger_id"]
    passenger_appear_time = request["appear_time"]
    current_route = vehicle["route"]
    current_tracker = vehicle["tracker"]
    
    # 🆕 获取约束参数
    max_detour_time = input_data.get("max_detour_time", float('inf'))
    max_waiting_time = input_data.get("max_waiting_time", float('inf'))
    max_travel_time_ratio = input_data.get("max_travel_time_ratio", float('inf'))
    
    # 🆕 计算直达时间作为基准
    get_travel_time = input_data["get_travel_time"]
    current_time = input_data["current_time"]
    direct_travel_time = get_travel_time(origin, destination, current_time)
        ...
        for o_pos in origin_positions:
            for d_pos in dest_positions:
                if o_pos < d_pos:
                    ...
                    if not _check_quality_constraints(
                        vehicle=vehicle,
                        ...
                        passenger_appear_time=passenger_appear_time,
                        direct_travel_time=direct_travel_time,
                        max_detour_time=max_detour_time,
                        max_waiting_time=max_waiting_time,
                        max_travel_time_ratio=max_travel_time_ratio,
                        input_data=input_data
                    ):
                        continue  # ❌ Quality constraints violated
...
def _check_quality_constraints(
    ...
    origin: str,
    destination: str,
    passenger_appear_time: float,
    direct_travel_time: float,
    max_detour_time: float,
    max_waiting_time: float,
    max_travel_time_ratio: float,
    input_data: Dict
) -> bool:
    """
    Check service quality constraints for a passenger insertion.
    
    Validates three constraints:
    1. Detour time: actual_travel_time - direct_travel_time ≤ max_detour_time
    2. Waiting time: pickup_time - appear_time ≤ max_waiting_time
    3. Travel time ratio: actual_travel_time / direct_travel_time ≤ max_travel_time_ratio
    
    Args:
        vehicle: Vehicle object with current state
        candidate_route: Proposed route after insertion
        candidate_tracker: Tracking info after insertion
        passenger_id: ID of passenger being inserted
        origin: Pickup station
        destination: Dropoff station
        passenger_appear_time: When passenger appeared in system
        direct_travel_time: Direct travel time from origin to destination
        max_detour_time: Maximum allowed detour time (seconds)
        max_waiting_time: Maximum allowed waiting time (seconds)
        max_travel_time_ratio: Maximum allowed ratio of actual/direct time
        input_data: Input data containing get_travel_time function
    ...
    for i, station in enumerate(candidate_route):
        if station == origin and pickup_idx is None:
            # Check if this station has our passenger in pickup list
            if passenger_id in candidate_tracker.get(station, {}).get("pickup", []):
                pickup_idx = i
        if station == destination and dropoff_idx is None:
            # Check if this station has our passenger in dropoff list
            if passenger_id in candidate_tracker.get(station, {}).get("dropoff", []):
                dropoff_idx = i
    
    if pickup_idx is None or dropoff_idx is None:
        logger.error(
            f"Cannot find pickup/dropoff positions for {passenger_id} "
            f"in route {candidate_route}"
        )
        return False
    
    # Calculate vehicle's arrival time at pickup station
    arrival_time_at_vehicle_start = current_time
    
    # Time to reach pickup station
    for i in range(pickup_idx):
        segment_time = get_travel_time(
            candidate_route[i], 
            candidate_route[i + 1], 
            arrival_time_at_vehicle_start
        )
        arrival_time_at_vehicle_start += segment_time
    
    pickup_time = arrival_time_at_vehicle_start
    
    # 🆕 CONSTRAINT 1: Waiting time check
    waiting_time = pickup_time - passenger_appear_time
    if waiting_time > max_waiting_time:
        logger.debug(
            f"Rejected {passenger_id}: waiting time {waiting_time:.1f}s "
            f"exceeds max {max_waiting_time:.1f}s"
        )
        return False
    
    # Calculate actual travel time (from pickup to dropoff)
    actual_travel_time = 0.0
    arrival_time = pickup_time
    
    for i in range(pickup_idx, dropoff_idx):
        segment_time = get_travel_time(
            candidate_route[i], 
            candidate_route[i + 1], 
            arrival_time
        )
        actual_travel_time += segment_time
        arrival_time += segment_time
    
    # 🆕 CONSTRAINT 2: Detour time check
    detour_time = actual_travel_time - direct_travel_time
    if detour_time > max_detour_time:
        logger.debug(
            f"Rejected {passenger_id}: detour time {detour_time:.1f}s "
            f"exceeds max {max_detour_time:.1f}s "
            f"(actual={actual_travel_time:.1f}s, direct={direct_travel_time:.1f}s)"
        )
        return False
    
    # 🆕 CONSTRAINT 3: Travel time ratio check
    if direct_travel_time > 0:  # Avoid division by zero
        travel_time_ratio = actual_travel_time / direct_travel_time
        if travel_time_ratio > max_travel_time_ratio:
            logger.debug(
                f"Rejected {passenger_id}: travel time ratio {travel_time_ratio:.2f} "
                f"exceeds max {max_travel_time_ratio:.2f} "
                f"(actual={actual_travel_time:.1f}s, direct={direct_travel_time:.1f}s)"
            )
            return False
    ...
    logger.debug(
        f"Quality check passed for {passenger_id}: "
        f"waiting={waiting_time:.1f}s, detour={detour_time:.1f}s, "
        f"ratio={actual_travel_time/direct_travel_time:.2f}"
    )
