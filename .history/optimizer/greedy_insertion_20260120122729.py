"""
optimizer/greedy_insertion.py (BEST-FIRST VERSION)

Greedy insertion algorithm with BEST-FIRST passenger selection.
At each iteration, selects the passenger that can be inserted with minimum cost increase.
...
4. SELECT BEST PASSENGER at each iteration (not first-come-first-served)
5. Clean and merge duplicate stations only in final output
"""
...
def greedy_insert_optimize(input_data: dict) -> Dict[str, List[Dict]]:
    ...
    Strategy:
    - At each iteration, evaluate ALL unassigned passengers
    - Select the one with minimum insertion cost (cost increase)
    - Assign that passenger to its best vehicle
    - Repeat until no more passengers can be assigned
    ...
    """
    logger.info("Starting BEST-FIRST greedy insertion optimization")
    ...
    # Track assigned/unassigned passengers
    assigned_passengers = set()
    remaining_indices = set(range(len(pending_requests)))
    
    iteration = 0
    
    # BEST-FIRST LOOP: Keep assigning until no feasible assignments remain
    while remaining_indices:
        ...
        # Evaluate EVERY unassigned passenger
            ...
            for vehicle in vehicles:
                ...
                if candidate_route is not None:
                    ...
                    logger.debug(
                        ...
                        f"cost_increase={cost_increase:.1f} "
                        f"(current={current_cost:.1f}, new={new_cost:.1f})"
                    )
        ...
        if best_passenger_idx is not None:
            ...
            logger.info(
                ...
                f"cost_increase={best_cost_increase:.1f}"
            )
        else:
    ...
    if len(assigned_passengers) < len(pending_requests):
        unassigned = [
            pending_requests[idx]["passenger_id"] 
            for idx in range(len(pending_requests))
            if pending_requests[idx]["passenger_id"] not in assigned_passengers
        ]
...
def _initialize_vehicles(minibuses: List[Dict]) -> List[Dict]:
    ...
    Internal format:
    {
        ...
        "initial_occupancy": current number of passengers,
        "route": [
...
def _compute_route_cost(route: List[Dict], input_data: Dict) -> float:
    ...
    Args:
        ...
        input_data: Contains get_travel_time function and current_time
        
    Returns:
        Total travel time in seconds (0.0 for empty routes)
    """
    if len(route) <= 1:
        return 0.0
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("GREEDY INSERTION - BEST-FIRST TEST")
    print("=" * 80)
    ...
    print("\nTest: 3 passengers (1 far, 2 close), 1 vehicle (capacity=2)")
