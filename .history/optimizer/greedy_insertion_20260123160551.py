...
def _simulate_arrival_times(route: List[Dict], vehicle: Dict, input_data: Dict) -> List[float]:
    """
    Return arrival_time at each stop in `route`.
    Includes travel from vehicle current_location -> first stop.
    """
    if not route:
        return []
    ...
    # If no travel time function, use fallback 300s per hop (consistent with your cost fallback)
    def tt(a, b, t):
        if get_travel_time is None:
            return 300.0
        try:
    ...
    # travel from current location to first stop
...
def _is_wait_and_detour_feasible(
    ...
    """
    Check max waiting time and max detour time for THIS passenger within this candidate route.
    waiting = pickup_time - appear_time
    detour  = (dropoff_time - pickup_time) - direct_time(origin,destination,pickup_time)
    """
    max_wait_time = float(input_data.get("max_wait_time", float("inf")))
    ...
    for i, stop in enumerate(merged):
        t = arrival_times[i]
        if passenger_id in stop["pickup"]:
            pickup_time = t
        if passenger_id in stop["dropoff"]:
            dropoff_time = t

    # must exist
    if pickup_time is None or dropoff_time is None or dropoff_time <= pickup_time:
        return False

    waiting = pickup_time - float(appear_time)
    if waiting > max_wait_time:
        logger.debug(
            f"  ✗ WAIT constraint: {passenger_id} waiting={waiting:.1f}s > {max_wait_time:.1f}s"
        )
        return False

    # detour needs direct time
    if get_travel_time is None:
        # Without travel time function, detour is not meaningful; treat as feasible or use fallback.
        # Here we use fallback 300s direct per hop -> 300s as a coarse estimate.
        direct = 300.0
    else:
        try:
            direct = float(get_travel_time(origin, destination, pickup_time))
        except:
            direct = 300.0

    onboard_segment = (dropoff_time - pickup_time)
    detour = onboard_segment - direct

    if detour > max_detour_time:
        logger.debug(
            f"  ✗ DETOUR constraint: {passenger_id} detour={detour:.1f}s > {max_detour_time:.1f}s "
...
def _is_candidate_feasible(
    ...
    """Capacity + waiting + detour feasibility for candidate insertion."""
