...
def greedy_insert_optimize(input_data: dict) -> Dict[str, List[Dict]]:
    ...
    # ========================================================================
    BATCH_SIZE = 20  # Process at most 30 passengers per call
...
def _try_insert_passenger(
    ...
    NEW: Checks max_detour_time constraint if specified in input_data.
    ...
    # Get max detour time constraint (if specified)
    max_detour_time = input_data.get("max_detour_time", None)
    get_travel_time = input_data["get_travel_time"]
    current_time = input_data["current_time"]
    
    # Calculate direct travel time for this passenger (if detour constraint exists)
    ...
    for pickup_pos in range(len(current_route) + 1):
        for dropoff_pos in range(pickup_pos + 1, len(current_route) + 2):
            ...
            if not _is_capacity_feasible(candidate, capacity, initial_occupancy):
                continue
            
            # NEW: Check detour time constraint
            if max_detour_time is not None:
                actual_travel_time = _calculate_passenger_travel_time(
                    candidate, passenger_id, origin, destination, 
                    get_travel_time, current_time
                )
                detour_time = actual_travel_time - direct_travel_time
                
                if detour_time > max_detour_time:
                    logger.debug(
                        f"  ✗ Detour too long: {detour_time:.1f}s > {max_detour_time:.1f}s "
                        f"(direct: {direct_travel_time:.1f}s, actual: {actual_travel_time:.1f}s)"
                    )
                    continue
...
def _calculate_passenger_travel_time(
    route: List[Dict],
    passenger_id: str,
    origin: str,
    destination: str,
    ...
    """
    Calculate actual travel time for a specific passenger in the route.
    
    This is the time from when they are picked up to when they are dropped off.
    
    Args:
        route: The candidate route
        passenger_id: The passenger to track
        origin: Passenger's origin station
        destination: Passenger's destination station
        get_travel_time: Function to get travel times
        current_time: Current simulation time
        
    Returns:
        Total travel time for this passenger
    """
    pickup_time = None
    dropoff_time = None
    ...
    for i, stop in enumerate(route):
        # Check if passenger is picked up at this stop
        if passenger_id in stop["pickup"] and stop["station"] == origin:
            pickup_time = arrival_time
        
        # Check if passenger is dropped off at this stop
        if passenger_id in stop["dropoff"] and stop["station"] == destination:
            dropoff_time = arrival_time
            break
        
        # Move to next stop
        if i < len(route) - 1:
            travel_time = get_travel_time(stop["station"], route[i+1]["station"], arrival_time)
            arrival_time += travel_time
    
    if pickup_time is None or dropoff_time is None:
        logger.warning(f"Could not find pickup/dropoff times for {passenger_id}")
        return float('inf')
