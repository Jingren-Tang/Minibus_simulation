...
def _convert_to_vehicle_objects(minibuses: List[Dict]) -> List[Dict]:
    ...
    🔧 Keep all stops, but validate tracker consistency
    """
    vehicles = []
    ...
    for mb in minibuses:
        ...
        # Build tracking info
        tracker = {}
        passengers_onboard = set(mb["passengers_onboard"])
        ...
        for stop in mb["current_route_plan"]:
            ...
            if stop["action"] == "PICKUP":
                ...
                # 🔧 Only keep dropoffs for passengers actually onboard
                valid_dropoffs = [
                    p for p in stop["passenger_ids"] 
                    if p in passengers_onboard
                ]
                if valid_dropoffs:
                    tracker[station]["dropoff"].extend(valid_dropoffs)
                
                # Log inconsistencies but don't fail
                invalid = set(stop["passenger_ids"]) - passengers_onboard
                if invalid:
                    logger.warning(
                        f"{mb['minibus_id']}: Ignoring DROPOFF for {invalid} "
                        f"at {station} (not onboard)"
                    )
