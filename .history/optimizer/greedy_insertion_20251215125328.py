...
def _try_insert_request(
    ...
    """
    Try to insert request (QUICK FIX VERSION)
    
    Current approach: Skip if origin or destination already exists in route
    This avoids duplicate stations but may miss optimization opportunities.
    
    TODO Phase 2: Implement proper station reuse logic
    ...
    # 🆕 QUICK FIX: Skip if origin or destination already in route
    # This prevents duplicate stations in the route
    if origin in current_route or destination in current_route:
        logger.debug(
            f"Skipping insertion for {passenger_id}: "
            f"{origin} or {destination} already in route {current_route}"
        )
    ...
    for i in range(len(current_route) + 1):
        for j in range(i + 1, len(current_route) + 2):
            ...
            # Build candidate tracker (deep copy + add new passenger)
            candidate_tracker = {}
