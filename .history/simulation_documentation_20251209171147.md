...
For production use, generates large-scale realistic demand. The total number of passengerin each 10 mins follows Poisson distribution, but they appear randomly during the 10 minutes.
...
The test version serves as a proof-of-concept for the mixed traffic system with the following configuration:

- **Duration**: 1 hour
- **Fleet composition**: 3 minibuses and 7 buses
- **Passenger generation**: Hardcoded test passengers
...
This version successfully demonstrates the core functionality of the discrete event simulation framework and the interaction between fixed-route buses and dynamic-route minibuses.

### Full-Scale Version (Bus-Only)

The full-scale version uses real-world data to validate the simulation engine's capability to handle realistic demand:
...
**Validation results**: The bus-only system successfully transported all passengers to their destinations during the first four hours of simulation. Only passengers generated in the final few seconds could not be served, which is expected behavior as they appeared too close to the simulation end time. This validates that:

1. The discrete event simulation engine handles realistic demand correctly
2. Real-world bus schedules integrate seamlessly with the simulation framework
3. The passenger generation from OD matrices produces realistic arrival patterns

### Next Steps

The development roadmap consists of two key phases:

**Phase 1: Minibus Integration**
...
This phased approach ensures that each component is validated independently before full system integration, reducing the risk of integration issues and facilitating debugging.
