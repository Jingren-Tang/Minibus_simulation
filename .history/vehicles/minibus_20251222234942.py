...
class Minibus:
    ...
    def get_minibus_info(self) -> Dict[str, Any]:
        ...
        ✅ UPDATED: Automatically filters out completed stops from route_plan
        to prevent duplicate assignments by the optimizer.
        ...
        Returns:
            ...
            cleaned route_plan (only pending stops)
        """
        # ✅ Filter route_plan to only include pending stops
        cleaned_route_plan = self._filter_pending_stops()
        ...
        return {
            ...
            "route_plan": cleaned_route_plan,  # ✅ Use cleaned plan instead of raw plan
    ...
    def _filter_pending_stops(self) -> List[Dict[str, Any]]:
        """
        Filter route_plan to remove already-completed stops.
        
        A stop is considered completed if:
        1. The minibus has already arrived at that station (current_location_id)
        2. AND the action has been executed:
        - For PICKUP: all passengers in passenger_ids are already onboard
        - For DROPOFF: all passengers in passenger_ids have already alighted
        
        This prevents the optimizer from seeing stale/duplicate tasks.
        
        Returns:
            List[Dict]: Filtered route_plan with only pending stops
        """
        if not self.route_plan:
            return []
        
        current_passenger_ids = {p.passenger_id for p in self.passengers}
        pending_stops = []
        
        for stop in self.route_plan:
            station_id = stop["station_id"]
            action = stop["action"]
            passenger_ids = set(stop["passenger_ids"])
            
            # Check if this stop is already completed
            is_completed = False
            
            if station_id == self.current_location_id:
                # We're at this station, check if action is already done
                if action == self.PICKUP:
                    # PICKUP is completed if ALL passengers are already onboard
                    if passenger_ids.issubset(current_passenger_ids):
                        is_completed = True
                        logger.debug(
                            f"{self.minibus_id}: Filtering completed PICKUP at {station_id} "
                            f"for passengers {passenger_ids}"
                        )
                
                elif action == self.DROPOFF:
                    # DROPOFF is completed if NO passengers are still onboard
                    if passenger_ids.isdisjoint(current_passenger_ids):
                        is_completed = True
                        logger.debug(
                            f"{self.minibus_id}: Filtering completed DROPOFF at {station_id} "
                            f"for passengers {passenger_ids}"
                        )
            
            # Keep the stop if not completed
            if not is_completed:
                pending_stops.append(stop)
        
        if len(pending_stops) < len(self.route_plan):
            logger.info(
                f"{self.minibus_id}: Filtered {len(self.route_plan) - len(pending_stops)} "
                f"completed stops, {len(pending_stops)} pending stops remain"
            )
