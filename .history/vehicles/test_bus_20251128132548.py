...
def test_passenger_boarding_and_alighting():
    ...
    # Create passengers with different destinations
    p1 = Passenger("P1", origin_id="A", destination_id="B", departure_time=0.0)
    p2 = Passenger("P2", origin_id="A", destination_id="C", departure_time=0.0)
    p3 = Passenger("P3", origin_id="A", destination_id="D", departure_time=0.0)
    p4 = Passenger("P4", origin_id="A", destination_id="E", departure_time=0.0)  # E not on route
    
    # Passengers arrive at station
    for p in [p1, p2, p3, p4]:
        p.arrive_at_station("A", 0.0)
    ...
    for p in result['boarded']:
        print(f"    • {p.passenger_id} → {p.destination_id}")
    print(f"  - Rejected: {len(result['rejected'])} passengers")
    for p in result['rejected']:
        print(f"    • {p.passenger_id} → {p.destination_id} (not on route)")
...
def test_bus_capacity():
    ...
    passengers = []
    for i in range(5):
        p = Passenger(f"P{i+1}", origin_id="A", destination_id="C", departure_time=0.0)
        p.arrive_at_station("A", 0.0)
        station_a.add_waiting_passenger(p)
        passengers.append(p)
...
def test_complete_journey():
    ...
    passengers_a = [
        Passenger("P1", "A", "B", 0.0),
        Passenger("P2", "A", "C", 0.0),
        Passenger("P3", "A", "D", 0.0),
    ]
    ...
    passengers_b = [
        Passenger("P4", "B", "C", 300.0),
        Passenger("P5", "B", "D", 300.0),
    ]
    for p in passengers_b:
        p.arrive_at_station("B", 300.0)
...
def test_edge_cases():
    ...
    print("\nTest 2: Alight passenger not on bus")
    p2 = Passenger("P_NOTHERE", "A", "B", 0.0)
    ...
    p3 = Passenger("P3", "A", "B", 0.0)
    p4 = Passenger("P4", "A", "C", 0.0)
