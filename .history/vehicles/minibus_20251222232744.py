...
class Minibus:
    ...
    def arrive_at_station(
        ...
        FIXED VERSION: Handles multiple consecutive stops at the same station
        by processing them all in one arrival event.
        ...
        Returns:
            Dict containing:
                ...
                - "action_type": "PICKUP" or "DROPOFF" or "BOTH"
        ...
        # ========================================================================
        # KEY FIX: Process ALL consecutive stops at this station
        # ========================================================================
        ...
        while (len(self.route_plan) > 0 and 
            ...
            logger.info(
                f"{self.minibus_id} processing stop {stops_processed + 1} at {station.station_id}: "
                f"{action_type} {len(passenger_ids)} passengers"
            )
            ...
            if action_type == self.PICKUP:
                ...
                action_types.append("DROPOFF")
                
            else:
                logger.error(f"Unknown action type: {action_type}")
            
            # Accumulate results
            all_boarded.extend(boarded)
            all_alighted.extend(alighted)
        ...
        if stops_processed > 1:
            logger.info(
                f"{self.minibus_id} processed {stops_processed} consecutive stops at {station.station_id}"
            )
        
        # Determine combined action type for reporting
        if "PICKUP" in action_types and "DROPOFF" in action_types:
            combined_action_type = "BOTH"
        elif "PICKUP" in action_types:
            combined_action_type = "PICKUP"
        elif "DROPOFF" in action_types:
            combined_action_type = "DROPOFF"
        else:
