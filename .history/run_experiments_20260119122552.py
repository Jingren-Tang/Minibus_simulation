...
class ExperimentRunner:
    ...
    def __init__(self, output_base_dir: str = "Minibus_ratio_results_5"):
        ...
        # Experiment parameters - UPDATED WITH OPTIMIZE_INTERVAL
        self.num_minibuses_values = [2, 3]
        self.minibus_capacity_values = [6, 8, 10]  # Changed from [6, 8]
        self.minibus_ratio_values = [0.3,0.4]
        # [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]  # Changed from [0.1, 0.2, 0.3]
        self.optimize_interval_values = [60, 120, 300, 600]  # Optimize interval in seconds (1min, 2min, 5min, 10min)
    ...
    def get_total_experiments(self) -> int:
        ...
        return (len(self.num_minibuses_values) * 
                ...
                len(self.optimize_interval_values))
        ...
        for num_minibuses in self.num_minibuses_values:
            for capacity in self.minibus_capacity_values:
                for ratio in self.minibus_ratio_values:
                    for optimize_interval in self.optimize_interval_values:
                        exp_config = {
                            'exp_id': exp_id,
                            ...
                            'exp_name': f"exp_{exp_id:03d}_n{num_minibuses}_c{capacity}_r{ratio}_opt{optimize_interval}",
                            'output_dir': str(self.output_base_dir / f"exp_{exp_id:03d}_n{num_minibuses}_c{capacity}_r{ratio}_opt{optimize_interval}")
                        }
    ...
    def modify_config(self, exp_config: Dict) -> None:
        ...
        try:
            ...
            # OPTIMIZE_INTERVAL (NEW)
            content = re.sub(
                r'OPTIMIZE_INTERVAL\s*=\s*\d+',
                f'OPTIMIZE_INTERVAL = {exp_config["optimize_interval"]}',
                content
            )
            ...
            logger.info(f"Config modified: n={num_minibuses}, "
                    ...
                    f"opt={exp_config['optimize_interval']}s, "
            ...
            if resume and exp_name in self.completed_experiments:
                ...
                if existing_metrics:
                    result = {
                        ...
                        'optimize_interval': exp_config['optimize_interval'],
                        'status': 'SUCCESS',
                        'error_message': None,
                        'runtime_seconds': 0.0,
                        **existing_metrics
    ...
    def save_summary(self, results: List[Dict]) -> None:
        ...
        try:
            ...
            # Reorder columns for better readability - UPDATED with optimize_interval
            column_order = [
                'exp_id', 'exp_name', 'status', 
                'num_minibuses', 'minibus_capacity', 'minibus_ratio', 'optimize_interval',
    ...
    def dry_run(self) -> None:
        ...
        for exp_config in experiments:
            ...
            print(f"  Minibuses: {exp_config['num_minibuses']}, "
                  ...
                  f"Optimize Interval: {exp_config['optimize_interval']}s")
